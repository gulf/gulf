...
describe('gulf', function () {
  ...
  describe('Linking to new documents', function () {
    ...
    before(function (cb) {
      ...
      doc.initializeFromStorage('abc').then(function () {
        docA = doc;
  ...
  describe('Linking to editable documents', function () {
    ...
    before(function (cb) {
      ...
      doc.initializeFromStorage(initialContent).then(function () {
        docA = doc;
        docB = new gulf.EditableDocument({ storageAdapter: new gulf.MemoryAdapter(), ottype: ottype });

        content = '';
        docB._onBeforeChange = function () {
      ...
      doc.initializeFromStorage(initialContent).then(function () {
        ...
        linkA = docA.slaveLink({
          authenticate: function authenticate(credentials) {
            return _promise2.default.resolve(credentials == 'rightCredentials');
          }
        });
        done();
      }).catch(done);
  ...
  describe('Linking to documents protected by write authorization', function () {
    ...
    beforeEach(function (cb) {
      ...
      doc.initializeFromStorage(initialContent).then(function () {
        docA = doc;
        docB = new gulf.EditableDocument({ storageAdapter: new gulf.MemoryAdapter(), ottype: ottype });
        docB._setContent = function (content) {
          return _promise2.default.resolve();
        };
        docB._onChange = function (cs) {
          return _promise2.default.resolve();
        ...
        linkA = docA.slaveLink({
          ...
          authorizeWrite: function authorizeWrite(msg, user) {
            return _promise2.default.resolve(msg[0] === 'requestInit' || msg[0] === 'ack');
          }
        });
        cb();
      }).catch(cb);
  ...
  describe('Linking documents in parallel environments', function () {
    ...
    before(function (cb) {
      ...
      master = require('child_process').fork(__dirname + '/helper/parallelmaster_fork.js', [initialContent], { silent: true });
