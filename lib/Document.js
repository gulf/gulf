...
Document.create = function(adapter, ottype, content, cb) {
  ...
  doc.content = ottype.deserialize? ottype.deserialize(content) : content
  doc.history.createDocument({contents: content, edit: Edit.newInitial(ottype)}, function(er) {
    if(er) return cb(er)
    cb(null, doc)
  })
}
...
Document.prototype.dispatchEdit = function(edit, fromLink, cb) {
  this.history.remembers(edit.id, function(er, remembers) {
    ...
    this.history.remembers(edit.parent, function(er, remembersParent) {
      if (!remembersParent) {
        fromLink.emit('editError', new Error('Edit "'+edit.id+'" has unknown parent "'+edit.parent+'"'))
        return cb
      }
      ...
      this.sanitizeEdit(edit, fromLink, function(er, edit) {
        ...
        this.history.storeSnapshot({id: edit.id, contents: this.content, edit: edit}, function(er) {
          if(er) {
            fromLink.emit('editError', er) // XXX: send this erro to the document
          }
          if(fromLink) fromLink.send('ack', edit.id)
